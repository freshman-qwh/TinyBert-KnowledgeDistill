...
def gelu(x):
    """gelu激活函数的实现。
        补充说明：OpenAI GPT的gelu略有不同（结果也略有不同）：
...
class HeadAttention(nn.Module):
    ...
    def transpose_for_scores(self, x):
        # 输入x: [batch, seq_len, all_head_size] -> [batch, heads, seq_len, head_size]
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        # attention_mask: [batch, 1, 1, seq_len] (已广播)
...
class BertConfig(object):
    ...
    def __init__(self,
        ...
        """
        # 支持两种初始化: 传入config文件路径或直接传入vocab_size
...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.word_embeddings = nn.Embedding(    # 词表嵌入
            config.vocab_size, config.hidden_size, padding_idx=0)
        self.position_embeddings = nn.Embedding(    # 位置嵌入
            config.max_position_embeddings, config.hidden_size)
        self.token_type_embeddings = nn.Embedding(  # 句子/片段嵌入
    ...
    def forward(self, input_ids, token_type_ids=None):
        # input_ids: [batch, seq_len], token_type_ids: [batch, seq_len]
        # 输出embeddings: [batch, seq_len, hidden]
        # 1. 自动生成位置 ID
...
class BertSelfAttention(nn.Module):
    ...
    def transpose_for_scores(self, x):
        # [batch, seq_len, all_head_size] -> [batch, heads, seq_len, head_size]
        new_x_shape = x.size()[
    ...
    def forward(self, hidden_states, attention_mask, output_att=False):
        ...
        # Softmax得到注意力概率
        """超级重点，注意力层对齐的其实是：KQ计算得到的 句子单词间 attention_probs 关系（没有V）"""
        attention_probs = nn.Softmax(dim=-1)(attention_scores)
...
class BertAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask):
        # 自注意力 + 残差输出
        """
        self_output (注意力上下文向量)[batch_size, seq_len, hidden_size]
        layer_att(注意力矩阵/地图)128*128,要学习这个！
        """
...
class BertLayer(nn.Module):
    def __init__(self, config):
        ...
        self.attention = BertAttention(config)  # 注意力层：计算注意力、线性映射归一化残差连接
        self.intermediate = BertIntermediate(config)    # 前馈神经网络：升维扩张特征
        self.output = BertOutput(config)    # 前馈神经网络：降维特征压缩，残差

    """
    重点，网络的一层前向传播。教师有12层，学生一般4或6层。
    包含：注意力层和前馈神经网络
    """
    def forward(self, hidden_states, attention_mask):
        # 单层Transformer: 自注意力 + FFN
...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        """重点，调试可打印self.layer大致模型结构"""
        self.layer = nn.ModuleList([BertLayer(config)
                                    for _ in range(config.num_hidden_layers)])

    def forward(self, hidden_states, attention_mask):
        # 堆叠多层Transformer，返回各层输出与注意力
        all_encoder_layers = []
        all_encoder_atts = []
        for _, layer_module in enumerate(self.layer):
            all_encoder_layers.append(hidden_states)
            """超级重点，编码层获取 表征层和注意力层 输出，用于对齐"""
...
class BertPooler(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # 取最后一层 Transformer 的输出。取 Batch 中每个序列的第一个 Token（即 [CLS]）。[CLS] 位置的向量已经充分融合了整句话的上下文信息
...
class BertLMPredictionHead(nn.Module):
    ...
    def forward(self, hidden_states):
        # 输出词表logits: [batch, seq_len, vocab_size]
...
class BertModel(BertPreTrainedModel):
    ...
    """重点，教师学生模型基本的bert架构，前向传播"""
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
                output_all_encoded_layers=True, output_att=True):
        # input_ids/token_type_ids/attention_mask: [batch, seq_len]
        # 输出: encoded_layers(各层隐藏状态列表), layer_atts(各层注意力), pooled_output([CLS]向量)
        ...
        # 扩展为可广播的注意力mask: [batch, 1, 1, seq_len]
        ...
        # embedding_output: [batch, seq_len, hidden]
        embedding_output = self.embeddings(input_ids, token_type_ids)   # BertEmbeddings的前向传播
        encoded_layers, layer_atts = self.encoder(embedding_output,
                                                  extended_attention_mask)  # BertEncoder的前向传播

        # 提取 [CLS]，线性变化+激活，做成一个专门代表整句话意思的特征向量。
        pooled_output = self.pooler(encoded_layers) # BertPooler的前向传播
...
class BertForPreTraining(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
        ...
        # sequence_output: [batch, seq_len, hidden], pooled_output: [batch, hidden]
...
class TinyBertForPreTraining(BertPreTrainedModel):
    def __init__(self, config, fit_size=768):
        ...
        self.bert = BertModel(config)   # 包含embedding、encode、pooler的实例
        self.cls = BertPreTrainingHeads(
            config, self.bert.embeddings.word_embeddings.weight)
    ...
    def forward(self, input_ids, token_type_ids=None,
        ...
        # 输出: att_output(各层注意力), sequence_output(各层表示并投影到fit_size)
        """重点，主要的bert架构流程"""
        ...
        """
        超级重点，通用蒸馏的分布对齐。
        学生中间层的维度要和教师对齐，一般教师中间层768维，学生312维
        线性映射
        """
...
class BertForMaskedLM(BertPreTrainedModel):
    ...
    该模块包括BERT模型和掩蔽语言建模首部。

    Params:
        config: 一个带有构建新模型配置的 BertConfig 类实例。
...
class BertForNextSentencePrediction(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, next_sentence_label=None):
        # NSP任务前向: pooled_output -> 二分类logits
